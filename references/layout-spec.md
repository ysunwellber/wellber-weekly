...
### 2026-09-25 · 手机顶部开孔避让层（v1.5.2）

手机看长图时图片按宽度撑满，顶部居中开孔（灵动岛 / 挖孔）会压住大标题上沿（实拍里压掉了
「威尔新资讯」的「新」字）。现在报头绿块顶部多出一段**纯色安全带**，开孔只能遮到纯色。

- **安全带高度**：`safeTop.height = 206`（1280 稿宽）= 最终 1744px 图上的 **280px**
  （= 最新 iPhone 灵动岛 62pt 安全区折算 269px 的上取整，覆盖各机型最坏情况）
- **实现**：直接把绿块的 `padding-top` 换成 `safeTop.height`，绿块自然长高、内部元素整体下移，
  **不新增容器层级**。设 `0` 即完整退回旧版
- **logo 直印**：安全带下方居中印 `assets/logo.png`（与页尾同一枚），**不加底座色块**，
  原色 `#27272a` 在苔藓绿上实测对比 **6.85:1**（反白只有 2.17:1，会发飘）

| 元素 | 规格 | 实测基准（行号相对绿块顶） |
|---|---|---|
| 避让安全带 | 纯苔藓绿 `#b2b382`，高 `safeTop.height`（暂 206 → 图上 280px） | 行 **0–205** |
| 报头 logo | `assets/logo.png`，宽 `headerLogo.width`（646 → 图上 880px，占图宽 50%），居中、原色 | 行 **238–310**（高 73px） |

启用后报头内部整体下移，下移量写进 `baseline.shift`（当前 **340**），`build.py` 会自动叠加到各校验窗口上。
...
**v1.5.2 起还有 `baseline.shift`**：手机开孔避让层造成的报头内部整体下移量（当前 **340**）。
`build.py` 会把 `shift` 自动加到 `barHeight` / `titleWindow` / `navWindow` / `navBandRows` 上
（日期是相对绿块**底部**量的，不加）。**调 `safeTop.height` 或 `headerLogo` 尺寸后，只需重算这一个数。**
