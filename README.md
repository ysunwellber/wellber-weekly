...
> **内容从哪来**：每期走定死的四关 —— ① 确认刊期（上期期号 → 本期期号 + 发布日期）
...
```jsonc
{
  "issue": 40,                          // 期号（= 上期 + 1，先确认再填）
  "date": "auto",                       // "auto" = 本周五；也可写死 "2026.9.25"
  "output": { "dir": "输出目录", "name": "" },   // name 留空则自动生成 40_0918
...
「上期期号 → 本期期号（+1）→ 发布日期（默认本周五）」。**期号要跟孙哥确认**；
头条报的是公司自己的事，AI 搜不到，也得问孙哥（他这期没有就跳过，第一条放行业新闻）。
